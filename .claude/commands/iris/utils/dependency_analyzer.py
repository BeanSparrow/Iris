#!/usr/bin/env python3
"""
Dependency Analysis Logic for IRIS Enhancement System

Analyzes feature dependencies and determines optimal insertion points
in existing sprint milestone structures. Uses SQLite database backend.
"""
...
def find_project_root(start_dir: str = None) -> str:
    """Find project root by looking for IRIS-specific markers, then .git directory
    ...
    Raises:
        ValueError: If no IRIS project markers are found.
    """
    ...
    while current != os.path.dirname(current):  # Not at filesystem root
        # Priority 1: Look for IRIS-specific markers (.tasks with database)
        tasks_dir = os.path.join(current, '.tasks')
        if os.path.exists(tasks_dir):
            # Check for IRIS database
            if os.path.exists(os.path.join(tasks_dir, 'iris_project.db')):
                return current
            # Also accept legacy task_graph.json for backwards compatibility
            if os.path.exists(os.path.join(tasks_dir, 'task_graph.json')):
                return current

        # Priority 2: Look for .git directory (likely a project root)
        if os.path.exists(os.path.join(current, '.git')):
            # Check if this git project also has IRIS files
            tasks_dir = os.path.join(current, '.tasks')
            if os.path.exists(tasks_dir):
                return current
            # If no .tasks, continue searching - this might be a parent repo

        # Priority 3: Look for .claude directory with iris commands
        claude_dir = os.path.join(current, '.claude', 'commands', 'iris')
    ...
    # No IRIS project found - provide helpful error
    raise ValueError(
        f"No IRIS project found starting from '{original_start}'. "
        f"Looking for directory containing '.tasks/iris_project.db' or '.tasks/'. "
        f"Make sure you're running from within an IRIS project directory."
    )
...
class DependencyAnalyzer:
    def __init__(self, tasks_dir: str = None):
        ...
        self.milestones = self._load_milestones_from_db()
        self.techstack = self._load_techstack_from_db()
        self.deferred = self._load_deferred_from_db()
    ...
    def _load_tasks_from_db(self) -> List[Dict]:
        ...
        try:
            with self.db.get_connection() as conn:
                tasks = conn.execute("""
                    SELECT id, milestone_id, title, description, status, order_index
    ...
    def _load_milestones_from_db(self) -> List[Dict]:
        """Load milestones from SQLite database"""
        try:
            with self.db.get_connection() as conn:
    ...
    def _load_techstack_from_db(self) -> Dict:
        """Load technology stack from SQLite database"""
        try:
            with self.db.get_connection() as conn:
                technologies = conn.execute("""
                    SELECT name, category, version, is_latest_stable, official_url,
                           decision_reason
                    FROM technologies
                """).fetchall()
                ...
                for tech in technologies:
                    stack[tech['name']] = {
                        ...
                        'is_latest_stable': bool(tech['is_latest_stable']),
                        'documentation': {'official_url': tech['official_url']},
                        'decision_reason': tech['decision_reason']
                    }
                return {'stack': stack}
        except Exception:
            return {}

    def _load_deferred_from_db(self) -> Dict:
        """Load deferred features from SQLite database"""
        try:
            with self.db.get_connection() as conn:
                deferred = conn.execute("""
                    SELECT id, name, reason, complexity_score, original_requirement, priority
                    FROM deferred_features
                """).fetchall()
                return {'deferred_features': [dict(d) for d in deferred]}
        except Exception:
            return {'deferred_features': []}
    ...
    def analyze_feature(self, feature_description: str) -> FeatureAnalysis:
        ...
        return FeatureAnalysis(
            feature_id=f"F-ENH-{len(self.tasks)+1:03d}",
