#!/usr/bin/env python3
"""
Research Integration System for Iris Enhancement

Integrates with existing Iris research system to perform targeted
...
class ResearchIntegrator:
    ...
    def _load_existing_research(self) -> Dict:
        """Load existing techstack research from database"""
        try:
            with self.db.get_connection() as conn:
                technologies = conn.execute("""
                    SELECT name, version, is_latest_stable, official_url,
                           last_verified, needs_verification, decision_reason
                    FROM technologies
                """).fetchall()
                ...
                if technologies:
                    ...
                    for tech in technologies:
                        stack[tech['name']] = {
                            ...
                            'needs_verification': bool(tech['needs_verification']),
    ...
    def update_techstack_with_research(
        ...
        """Update techstack research with new findings - saves to database"""
                ...
                try:
                    with self.db.get_connection() as conn:
                        conn.execute("""
                            ...
                            (name, category, version, is_latest_stable, official_url, last_verified,
                             needs_verification, decision_reason)
                        ...
                        """, (
                            ...
                            official_url,
                            False,
                            f"Enhancement research by {result.agent_id}: {', '.join(result.recommendations[:2])}"
                        ))

                        # Save technology sources
                        for source in result.sources[:3]:
                            conn.execute("""
                                INSERT INTO technology_sources
                                (technology_name, source_url, published_date, relevance, notes)
                                VALUES (?, ?, date('now'), ?, ?)
                            """, (
                                result.technology,
                                source,
                    ...
                    print(f"Warning: Failed to save technology to database: {e}")

                # Also update in-memory dict for backwards compatibility
